...
export const SettingsPage: React.FC = () => {
  ...
  const handleToggle = async (key: string, label: string) => {
    ...
    try {
      ...
      showErrorToast(err?.message || "Failed to update setting");
    }
  };
  ...
  const renderToggleSwitch = (key: string, label: string, isChecked: boolean, disabled: boolean = false) => (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden ${disabled ? "cursor-not-allowed opacity-50" : "cursor-pointer"
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 pb-3 text-xs font-semibold transition-all cursor-pointer whitespace-nowrap relative ${isActive
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`rounded-md px-2 py-0.5 text-[10px] font-bold ${is2FaEnabled
